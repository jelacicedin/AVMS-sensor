...
const int stepsPerMovePhi = 20;
const int stepsPerMoveTheta = 20;

// Set the speed to 5 rpm:
char stepper_speed = 5;
  

const int phiWindow = stepsPerRevolution / 3 + stepsPerRevolution / 8 ;
...
// Home switch pins:
const int homeSwitchBase = 2;
const int homeSwitchArm = 3;
...
void setup() {
  ...
  // homeswitch pins:
  pinMode(homeSwitchBase, INPUT_PULLUP);
  pinMode(homeSwitchArm, INPUT_PULLUP);
}
...
// Move motors to home position (base 0, arm = stepsPerRevolution/4)
  ...
  if (goHome) {
    // Home arm:
    while (digitalRead(homeSwitchArm)) {
      motor_arm.step(1);
    }
    motor_arm.step(-150);
    ...
    // Home base:
    motor_base.setSpeed(2);
    while (digitalRead(homeSwitchBase)) {
      motor_base.step(-1);
    }
    motor_base.setSpeed(stepper_speed);
    motor_base.step(280);
