...
namespace UnitTestsOpenTK.LinearAlgebra
{
  public static class StandardMatrixTests
    {
        public static bool IsUpperTriangular(Matrix3d m)
        {
        ...
        public static bool IsLowerTriangular(Matrix3d m)
        {
            for (int iRow = 0; iRow < m.RowLength(); iRow++)
            {
                for (int iCol = 0; iCol < m.ColumnLength(); iCol++)
        ...
        public static bool IsDiagonal(Matrix3d m)
        {
            for (int iRow = 0; iRow < m.RowLength(); iRow++)
            {
                for (int iCol = 0; iCol < m.ColumnLength(); iCol++)
        ...
        public static bool IsSymetric(Matrix3d m)
        {
        ...
        public static  bool IsNonnegativeDiagonal(Matrix3d mat)
        {
        ...
        public static bool IsIntegerValued(Matrix3d mat)
        {
            for (int i = 0; i < mat.RowLength(); i++)
            {
                for (int j = 0; j < mat.ColumnLength(); j++)
