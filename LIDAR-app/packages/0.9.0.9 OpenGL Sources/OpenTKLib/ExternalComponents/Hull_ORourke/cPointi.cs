...
namespace OpenTKLib
{
    ...
    public class cPointi
    {
        ...
        {
            double l =Convert.ToSingle( Math.Sqrt(Math.Pow((p.X - p1.X), 2) + Math.Pow((p.Y - p1.Y), 2)));
            return l;
        }

        public double Dist(cPointi p) //returns the distance of two points
        {
            double l = Convert.ToSingle(Math.Sqrt(Math.Pow((p.X - this.X), 2) + Math.Pow((p.Y - this.Y), 2)));
            return l;
        }
