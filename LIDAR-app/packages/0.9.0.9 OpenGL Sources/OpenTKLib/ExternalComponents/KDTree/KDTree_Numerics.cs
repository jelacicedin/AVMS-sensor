...
namespace OpenTKLib
{
        ...
        where T : IEquatable<T>
    {
        ...
        //public List<Vector3d<T>> points;
        public List<Vector3d<T>> points;
        public BoundingBox<T> Counter;

        public KDTree3D_Numerics(List<NLinear.Vector3d<T>> Points, BoundingBox<T> box)
        {
            points = Points; Counter = box;
        }
        public KDTree3D_Numerics(List<Vector3d<T>> Points)
        {
            this.points = new List<Vector3d<T>>();
            T x1, x2, y1, y2, z1, z2;
        ...
        public List<KDTree3D_Numerics<T>> Split()
        {
            ...
            List<Vector3d<T>> ps1 = new List<Vector3d<T>>();
            List<Vector3d<T>> ps2 = new List<Vector3d<T>>();
        ...
        private static int CompareVectors_X(Vector3d<T> first, Vector3d<T> second)
        {
            if (first == default(Vector3d<T>)) { if (second == default(Vector3d<T>)) { return 0; } else { return -1; } }
            else
            {
                if (second == default(Vector3d<T>)) { return 1; }
                else
                {
        ...
        private static int CompareVectors_Y(Vector3d<T> first, Vector3d<T> second)
        {
            if (first == default(Vector3d<T>) ) { if (second == default(Vector3d<T>)) { return 0; } else { return -1; } }
            else
            {
        ...
        private static int CompareVectors_Z(Vector3d<T> first, Vector3d<T> second)
        {
            if (first == default(Vector3d<T>)) { if (second == default(Vector3d<T>)) { return 0; } else { return -1; } }
            else
            {
        ...
        public bool IsPointin(Vector3d<T> p, BoundingBox<T> pl)
        {
        ...
        public static bool IsPointinBox(Vector3d<T> p, BoundingBox<T> pl)
        {
            Numeric<T> x1 = pl.Min.X;
            Numeric<T> y1 = pl.Min.Y;
            Numeric<T> z1 = pl.Min.Z;
        ...
        public static List<BoundingBox<T>> SolveKdTree3D(List<Vector3d<T>> points, BoundingBox<T> box)
        {
            ...
            List<Vector3d<T>> pts = new List<Vector3d<T>>();
            points.ForEach(delegate(Vector3d<T> p)
            {
        ...
        public static List<BoundingBox<T>> SolveKdTree3D(List<Vector3d<T>> points)
        {
            List<BoundingBox<T>> PL = new List<BoundingBox<T>>();
            if (points.Count < 1) return PL;
            List<KDTree3D_Numerics<T>> trees = new List<KDTree3D_Numerics<T>>();
        ...
        public void Dispose()
        {
            this.points = default(List<Vector3d<T>>);
