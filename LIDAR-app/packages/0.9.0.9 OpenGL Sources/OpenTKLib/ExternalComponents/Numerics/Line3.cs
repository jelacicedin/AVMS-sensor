...
namespace NLinear
{
        ...
        where T : IEquatable<T>
    {
        ...
        private Vector3d<T> from;
        private Vector3d<T> to;
        Vector3d<T> dir;
       

        public Line3(Vector3d<T> p0, Vector3d<T> p1, T unit)
        {
            from = p0;
            to = p1;
        ...
        public Line3(Vector3d<T> p0, Vector3d<T> p1)
        {
            from = p0;
            to = p1;
        ...
        void Set(Vector3d<T> p0, Vector3d<T> p1, T unit)
        {
            from = p0; 
            dir = p1 - p0;
            dir.Normalize(unit);
        ...
        public Vector3d<T> ClosestPointTo(Vector3d<T> point)
        {
            return ((point - from) ^ dir) * dir + from;
        }

        public Vector3d<T> GetPoint(T parameter)
        {
            return from + dir * parameter;
        }

        public T DistanceTo(Vector3d<T> point, T unit)
        {
        ...
        {
            return EqualityComparer<Vector3d<T>>.Default.Equals(v1.dir, v2.dir)
                && EqualityComparer<Vector3d<T>>.Default.Equals(v1.from, v2.from);
        }
        ...
        public static bool intersection(Line3<T> a, Line3<T> b, ref Vector3d<T> intersectionPoint)
        ...
        // in 3d; will also work in 2d if z components are 0
        {
            ...
            //Check with
            T v = a.DistanceTo(b);
            double valRet = Convert.ToSingle(v);
            //if(val > 0)
            //should give the same result as below
            

            Vector3d<T> da = a.To - a.From;
            Vector3d<T> db = b.To - b.From;
            Vector3d<T> dc = b.From - a.From;
           ...
           // Point s = dot(cross(dc, db), cross(da, db)) / norm2(cross(da, db));
             Vector3d<T> temp1 = dc.Cross(db);
            Vector3d<T> temp2 = da.Cross(db);
            val = temp1.Dot(temp2);
            Numeric<T> valN = val;
            valN /= temp2.Norm2();
            double vald = Convert.ToSingle( val);
            if (vald >= 0.0 && vald <= 1)
            {
