...
namespace NLinear
{
    public class HullFrame
    {
        ...
        public List<Line3<double>> ComputeVoronoi3d(List<Line3<double>> myLines, List<Vector3d<double>> myPCL)
        {
            ...
            */
            ...
            {
                Vector3d<double> pt = myPCL[iii];
                Hull h = new Hull(hu, pt);
                for (int i = 0; i < myPCL.Count; i++)
                {
                    double t = myPCL[i].DistanceTo(pt, 1);
                    if (t > 0.001 && t < h.R * 2)
                    {
                        Vector3d<double> cen = new Vector3d<double>(pt); 
                        cen += myPCL[i]; cen /= 2;
                        Vector3d<double> v = pt - myPCL[i];
        ...
        //        }
        //    }
        //    Vector3d<double> cen = new Vector3d<double>();
        ...
        //        Vector3d<double> vi0 = cp[0];

        //        Vector3d<double> xi0 = x[i][0];


        //        Plane3<double> p = new Plane3<double>(x[i][0], x[i][1], x[i][2], 1);
                
        //        Vector3d<double> v = cen - p.ClosestPoint(cen);
        //        v.Normalize(1);
        ...
        //    }
        //    List<Vector3d<double>> pt = new List<Vector3d<double>>();
