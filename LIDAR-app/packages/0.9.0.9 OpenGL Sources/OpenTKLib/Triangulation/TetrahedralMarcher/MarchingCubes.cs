...
namespace OpenTKLib
{
    ...
    public class MarchingCubes : RenderableObject
    {
        ...
        void vMarchingCubes()
        {
            ...
            double fStepSize = 1f/ iDataSetSize;
        ...
        void MarchCube(double fX, double fY, double fZ, double fScale)
        {
            ...
            Vector3d[] edgeVectors = new Vector3d[12];
            Vector3d[] edgeNormals = new Vector3d[12];
        ...
        void MarchCubeTetrahedron(double fX, double fY, double fZ, double fScale)
        {
            int iVertex, iTetrahedron, iVertexInACube;
            Vector3d[] asCubePosition = new Vector3d[8];
            double[] afCubeValue = new double[8];
            Vector3d[] asTetrahedronPosition = new Vector3d[4];
            double[] afTetrahedronValue = new double[4];
        ...
        void MarchTetrahedron(Vector3d[] pasTetrahedronPosition, double[] pafTetrahedronValue)
        {
            ...
            double fOffset, fInvOffset;//, fValue = 0.0f;
            Vector3d[] edgeVertex = new Vector3d[6];
            Vector3d[] edgeNorm = new Vector3d[6];
            //Vector3d sColor;
        ...
        void generateColor(Vector3d rfColor, Vector3d rfPosition, Vector3d rfNormal)
        {
        ...
        void normalizeVector(Vector3d rfVectorResult, Vector3d rfVectorSource)
        {
            double fOldLength;
            double fScale;
        ...
        double distanceToPoints(double fX, double fY, double fZ)
        {
            double fResult = 0.0f;
            double fDx, fDy, fDz;
            fDx = fX - vectors[0].X;
        ...
        double distanceToLines(double fX, double fY, double fZ)
        {
            double fResult = 0.0f;
            double fDx, fDy, fDz;
            fDx = fX - vectors[0].X;
        ...
        double height(double fX, double fY, double fZ)
        {
            double fHeight = 20.0f * Convert.ToSingle((fTime + Math.Sqrt((0.5f - fX) * (0.5f - fX) + (0.5f - fY) * (0.5f - fY))));
            fHeight = 1.5f + 0.1f * Convert.ToSingle((Math.Sin(fHeight) + Math.Cos(fHeight)));
            double fResult = (fHeight - fZ) * 50.0f;
        ...
        Vector3d getNormal( double fX, double fY, double fZ)
        {
        ...
        void testapp()
        {
            double[] afPropertiesAmbient = { 0.50f, 0.50f, 0.50f, 1.00f };
            double[] afPropertiesDiffuse = { 0.75f, 0.75f, 0.75f, 1.00f };
            double[] afPropertiesSpecular = { 1.00f, 1.00f, 1.00f, 1.00f };
        ...
        //void vMarchingCubes();
        //void vMarchCube1(double fX, double fY, double fZ, double fScale);
        //void vMarchCube2(double fX, double fY, double fZ, double fScale);
        //void(*vMarchCube)(double fX, double fY, double fZ, double fScale) = vMarchCube1;
        ...
        //{
        //    double fAspect, fHalfWorldSize = (1.4142135623730950488016887242097f / 2);
