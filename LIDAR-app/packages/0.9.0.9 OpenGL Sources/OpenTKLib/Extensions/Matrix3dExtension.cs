...
namespace OpenTKLib
{
    ...
    public static class Matrix3dExtension
    {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    if (d > threshold && d1 > threshold)
                    {
                        ...
                        System.Diagnostics.Debug.Assert(false, d.ToString() + " -- Matrices are not equal, element : " + i.ToString() + " : " + j.ToString() + " : Values: " + mat[i, j].ToString() + " : " + other[i, j].ToString());
                        return false;
                    }
                }
            }
        ...
        /// <param name="targetV"></param>
        /// <returns></returns>
        public static Matrix3d RotationOneVectorToAnother(this Matrix3d mat, Vector3d sourceV, Vector3d targetV)
        {
            ...
            Vector3d u = p - sourceV;
            Vector3d v = p - targetV;
        ...
        /// <param name="targetV"></param>
        /// <returns></returns>
        public static Matrix3d RotationCoordinateChange(this Matrix3d mat, PointCloudVertices cloudOrigin, PointCloudVertices target)
        {
            for (int i = 0; i < 3; i++)
            {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    mat[i, j] = Vector3d.Dot(cloudOrigin[i].Vector, target[j].Vector);
                }
            }
            //    mat.Transpose();
            //mat.Invert();
