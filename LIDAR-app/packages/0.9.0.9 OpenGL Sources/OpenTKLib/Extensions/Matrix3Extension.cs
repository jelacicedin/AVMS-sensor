    ...
    public static class Matrix3Extension
    {
        ...
        public static bool CompareMatrices(this Matrix3 mat, Matrix3 other, float threshold)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    float d = Math.Abs(mat[i, j] - other[i, j]);
                    if (d > threshold)
                    {

                        System.Diagnostics.Debug.Assert(false, "Matrices are not equal, element : " + i.ToString() + " : " + j.ToString() + " : Values: " + mat[i, j].ToString() + " : " + other[i, j].ToString());
                        return false;
                    }
                }
            }
        ...
        public static Matrix3 Rotation_ToOriginAxes(this Matrix3 mat, PointCloudVertices cloudOrigin)
        {
        ...
        /// <param name="targetV"></param>
        /// <returns></returns>
        public static Matrix3 RotationCoordinateChange(this Matrix3 mat, PointCloudVertices cloudOrigin, PointCloudVertices target)
        {
            for (int i = 0; i < 3; i++)
            {
            ...
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    mat[i, j] = Convert.ToSingle(Vector3d.Dot(cloudOrigin[i].Vector, target[j].Vector));
                }
            }
            //    mat.Transpose();
            //mat.Invert();
