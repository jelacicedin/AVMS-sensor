...
namespace MIConvexHull
{
    ...
    internal class ConvexHullInternal
    {
        ...
        int DetermineDimension()
        {
            ...
            for (var i = 0; i < 10; i++)
                dimensions.Add(InputVertices[r.Next(VCount)].Vertex.PositionArray.Length);
        ...
        /// <summary>
        /// Create the first faces from (dimension + 1) pointCloud.
        /// </summary>
        /// <returns></returns>
        ConvexFaceInternal[] InitiateFaceDatabase()
        {
            ...
            {
                var pointCloud = ConvexHull.Where((_, j) => i != j).ToArray(); // Skips the i-th vertex
                var newFace = new ConvexFaceInternal(Dimension, new VertexBuffer());
                newFace.Vertices = pointCloud;
                Array.Sort(pointCloud, VertexWrapComparer.Instance);
        ...
        void UpdateAdjacency(ConvexFaceInternal l, ConvexFaceInternal r)
        {
            ...
            // mark all pointCloud on the 2nd face
        ...
        private bool CreateCone()
        {
            ...
            for (int fIndex = 0; fIndex < AffectedFaceBuffer.Count; fIndex++)
            {
                ...
                for (int i = 0; i < updateCount; i++)
                {
                    ...
                    int oldVertexIndex;
                    VertexWrap[] pointCloud;

                    newFace = ObjectManager.GetFace();
                    pointCloud = newFace.Vertices;
                    for (int j = 0; j < Dimension; j++) pointCloud[j] = oldFace.Vertices[j];
                    oldVertexIndex = pointCloud[forbidden].Index;
                    ...
                    if (currentVertexIndex < oldVertexIndex)
                    {
                        ...
                        {
                            if (pointCloud[j].Index > currentVertexIndex) pointCloud[j + 1] = pointCloud[j];
                            else
                            {
                    ...
                    pointCloud[orderedPivotIndex] = CurrentVertex;
        ...
        void HandleSingular()
        {
            ...
            // This means that all the affected faces must be on the hull and that all their "pointCloud beyond" are singular.
            for (int fIndex = 0; fIndex < AffectedFaceBuffer.Count; fIndex++)
            {
                var face = AffectedFaceBuffer[fIndex];
        ...
        /// <summary>
        /// Wraps the pointCloud and determines the dimension if it's unknown.
        /// </summary>
        /// <param name="pointCloud"></param>
        /// <param name="dim"></param>
        private ConvexHullInternal(IEnumerable<IVector> pointCloud)
        {
            InputVertices = new List<VertexWrap>(pointCloud.Select((v, i) => new VertexWrap { Vertex = v, PositionData = v.PositionArray, Index = i }));
            Dimension = DetermineDimension();
            Initialize();
        }

        /// <summary>
        /// Finds the pointCloud on the convex hull and optionally converts them to the TVertex array.
        /// </summary>
        /// <typeparam name="TVertex"></typeparam>
        /// <param name="onlyCompute"></param>
        /// <returns></returns>
        private IEnumerable<TVertex> GetConvexHullInternal<TVertex>(bool onlyCompute = false) where TVertex : IVector
        {
            ...
            where TVertex : IVector
        {
            ...
            {
                var face = faces[i];
                var pointCloud = new TVertex[Dimension];
        ...
        internal static List<ConvexFaceInternal> GetConvexFacesInternal<TVertex, TFace>(IEnumerable<TVertex> data)
            ...
            where TVertex : IVector
        {
            ConvexHullInternal ch = new ConvexHullInternal(data.Cast<IVector>());
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal static Tuple<IEnumerable<TVertex>, IEnumerable<TFace>> GetConvexHullAndFaces<TVertex, TFace>(IEnumerable<IVector> data)
            where TFace : ConvexFace<TVertex, TFace>, new()
            where TVertex : IVector
        {
